...
func xmlMarshal(v XMLElement) ([]byte, error) {
	if len(v.Attr) > 0 {
		newQueryAttrs := []xml.Attr{}
		for _, queryAttr := range v.Attr {
			if queryAttr.Name.Local != "xmlns" {
				newQueryAttrs = append(newQueryAttrs, queryAttr)
			}
		}
		v.Attr = newQueryAttrs
	}
